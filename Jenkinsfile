      ...
      steps {
        container('logstash') {
          ...
          docker.withRegistry("https://${DOCKER_REGISTRY}", 'artifactory-takumin') {
            sh "docker push ${DOCKER_REGISTRY}/${APP_NAME}:${GIT_COMMIT[0..10]}"
        ...
        container('maven') {
          withCredentials([usernamePassword(credentialsId: 'artifactory-takumin', variable: 'CREDS', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
            sh """
              ...
              helm repo update
              helm upgrade lead-dashboard liatrio-artifactory/lead-dashboard --version 0.0.4 --namespace toolchain --set logstash-jira.image.tag=${GIT_COMMIT[0..10]} --set elasticsearch.volumeClaimTemplate.storageClassName=gp2
              """
       }
        }
      }
